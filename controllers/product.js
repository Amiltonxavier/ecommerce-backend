...
exports.remove = (req, res) => {
	...
	product.remove((err, deletedProduct) => {
		...
		res.json({
			message: "Produto apadado com successo"
		})
	})
}
...
/**
* sell / arrival
* by sell = /products?sordBy=sold&order=desc&limit=4
* by arrival = /products?sordBy=creaAt&order=desc&limit=4
...
/**
* lista os produtos por pesquisa
 ...
 * conforme o usuário clica nessas caixas de seleção e botões de opção
 * faremos a solicitação da API e mostraremos os produtos aos usuários com base no que ele deseja
 */
...
exports.listSearch = (req, res) =>{
	...
	if(req.query.search){
		...
		if(req.query.category && req.category !=  'All'){
			query.category = req.query.category
		}

		//fined the product based on query object with 2 properties
		//search and category
...
exports.decreaseQuantity = (req, res, next) => {
	let bulkOps = req.body.order.products.map(item => {
		return {
			updateOne: {
				filter: { _id: item._id },
				//Incremento e Decremento da quantidade no banco
